function [result, edges] = texture_flattening(src_im,tgt_im,msk_im)

gray_image = im2gray(tgt_im);
edges = edge(gray_image,'canny'); %detect edges

msk_im=msk_im==1; 
tgt_im=double(tgt_im);
src_im=double(src_im); % for a higher precision

n=size(find(msk_im==1),1); %find the number of unknown pixels

msk_pixel_num=zeros(size(msk_im)); % convert mask to 1D array

msk_index=0;
for x=1:size(msk_pixel_num,1)
    for y=1:size(msk_pixel_num,2)
        if msk_im(x,y)==1 
            msk_index=msk_index+1;
            msk_pixel_num(x,y)=msk_index;  %convert mask(x,y) to unknown pixel
        end
    end
end
grad_gradients=[0 1 0; 1 -4 1; 0 1 0]; %gradients
result=tgt_im; %paste the edited region on traget image
for i=1:3 %loop through RGB channel
    sparse=5;
    
    A=spalloc(n,n,n*sparse);  %create the sparse matrix, at most have n*5 nonzero elements to it
    
    B=zeros(n,1); % column vector b
    grad=conv2(src_im(:,:,i),-grad_gradients,'same');  %gradients of the source image
    msk_index=0;
    for x=1:size(msk_pixel_num,1)
        for y=1:size(msk_pixel_num,2)
           if msk_im(x,y)==1
                msk_index=msk_index+1;
                A(msk_index,msk_index)=4; % diagnal always equal to 4
                
                if msk_im(x-1,y)==0 % if the left pixel of current pixel is known
                    B(msk_index)=tgt_im(x-1,y,i); % B takes the value of target image
                else % if unknown 
                    A(msk_index,msk_pixel_num(x-1,y))=-1; %A matrix will A(x-1,y) will be set to -1
                end
                if msk_im(x+1,y)==0 %same check for right pixel
                    B(msk_index)=B(msk_index)+tgt_im(x+1,y,i); 
                else 
                    A(msk_index,msk_pixel_num(x+1,y))=-1; 
                end
                if msk_im(x,y-1)==0 %pixel below
                    B(msk_index)=B(msk_index)+tgt_im(x,y-1,i); 
                else 
                    A(msk_index,msk_pixel_num(x,y-1))=-1;
                end
                if msk_im(x,y+1)==0 %pixel above
                    B(msk_index)=B(msk_index)+tgt_im(x,y+1,i); 
                else 
                    A(msk_index,msk_pixel_num(x,y+1))=-1; 
                end
                
                if edges(x,y)==1  %update the B vector with the gradients value
                    B(msk_index) = B(msk_index)+grad(x,y);
                else
                    for delta=-1:2:1
                        if edges(x+delta,y) == 1
                                B(msk_index) = B(msk_index) + tgt_im(x,y,i) - tgt_im(x+delta,y,i);
                        end

                        if edges(x,y+delta) == 1
                                B(msk_index) = B(msk_index) + tgt_im(x,y,i) - tgt_im(x,y+delta,i);  %edge detection, if edge exists, add to B matrix
                        end
                    end
                end
            end
        end
    end
    
    X=A\B;     %solve the linear system of equation
    for msk_index=1:length(X)    
        [index_x,index_y]=find(msk_pixel_num==msk_index);
        result(index_x,index_y,i)=X(msk_index);  %reshape X to 2D
    end
    
end

result=uint8(result);

end