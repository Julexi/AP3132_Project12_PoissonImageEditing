function result = seamless_tiling(src_im,tgt_im)

[w,h,x]=size(src_im);
msk_im = zeros(w,h);
msk_im(2:w-1,2:h-1) = 1; %here mask is the whole source/target image

msk_im=msk_im==1;
tgt_im=double(tgt_im);
src_im=double(src_im); % for higher precision

result=tgt_im; %initialize the result as target image
...
for i=1:3 %loop through RGB channel
    ...
    grad=conv2(src_im(:,:,i),-grad_gradients, 'same'); %calculate the gradients of source image
    msk_index=0;
    for x=1:size(msk_pixel_num,1)
        for y=1:size(msk_pixel_num,2)
            if msk_im(x,y)==1
